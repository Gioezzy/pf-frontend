...
interface RegistrationsTableCardProps {
  ...
  pendingTitles: Record<string, ChampionTitle>
  savingId: string | null
  onTitleChange: (registrationId: string, title: ChampionTitle) => void
  onSave: (registrationId: string) => void
}
            ...
            <tbody className="divide-y divide-gray-100">
              {registrations.map((registration, index) => {
                const currentSelection =
                  pendingTitles[registration.id] ??
                  registration.championTitle ??
                  "NONE"
                const isDirty =
                  currentSelection !== (registration.championTitle ?? "NONE")
                  ...
                  <tr key={registration.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <Select
                        ...
                        disabled={isSaving}
                        onValueChange={(value) =>
                          onTitleChange(registration.id, value as ChampionTitle)
                        }
                      >
                        <SelectTrigger className="w-[160px] h-8 text-xs bg-white border-gray-200 mx-auto">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 items-center">
                        ...
                        <Button
                          ...
                          className={isDirty ? "bg-[#5C7C99] hover:bg-[#4A647B] text-white shadow-none h-8 text-xs px-3" : "shadow-none border-gray-200 text-gray-600 h-8 text-xs px-3"}
                          disabled={!isDirty || isSaving}
                          onClick={() => onSave(registration.id)}
                        >
                          {isSaving && <Loader2Icon className="mr-2 size-3 animate-spin" />}
                          Simpan
                        </Button>
                      </div>
                    </td>
            ...
            return (
              <div key={registration.id} className="p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 mt-1">
                  <span className="text-sm font-medium text-gray-700">Tetapkan Gelar Juara:</span>
                  <div className="flex gap-2 w-full">
                    ...
                    <Button
                      size="icon"
                      variant={isDirty ? "default" : "outline"}
                      className={isDirty ? "bg-[#5C7C99] hover:bg-[#4A647B] text-white shadow-none w-10 shrink-0" : "shadow-none border-gray-200 text-gray-600 w-10 shrink-0"}
                      disabled={!isDirty || isSaving}
                      onClick={() => onSave(registration.id)}
                    >
                      {isSaving ? <Loader2Icon className="size-4 animate-spin" /> : <ClipboardCheckIcon className="size-4" />}
                    </Button>
                    <Button
                      size="icon"
                      variant="outline"
                      className="w-10 shrink-0 text-gray-500 border-gray-200 shadow-none"
                      onClick={() => setDetailModal(registration)}
                    >
                      <EyeIcon className="size-4" />
                    </Button>
